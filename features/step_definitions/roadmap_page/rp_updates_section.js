...
module.exports = function () {
  this.Then(/^there should be 6 update articles visible$/, () => {
    const articleBoxes = browser.elements('body > main > div > div.grid > article:nth-child(n)')

    expect(articleBoxes.value.length).to.equal(6)
  })

  this.Then(/^there should be 6 primary badges visible$/, () => {
    const primaryBadges = browser.elements('body > main > div > div.grid > article:nth-child(n) > div.box__inner > div.badge > div.badge__primary')

    expect(primaryBadges.value.length).to.equal(6)
  })

  this.Then(/^there should be 6 clickable primary badges to the Roadmap page$/, () => {
    const primaryLinks = browser.elements('body > main > div > div.grid > article:nth-child(n) > div.box__inner > div.badge > div.badge__primary > a[href="/en/roadmap"]')
  ...
  this.Then(/^there should be 6 secondary badges visible$/, () => {
    const secondaryBadges = browser.elements('body > main > div > div.grid > article:nth-child(n) > div.box__inner > div.badge > div.badge__secondary > a')
  ...
  this.Then(/^there should be 6 clickable article titles visible$/, () => {
    const articleTitles = browser.elements('body > main > div > div.grid > article:nth-child(n) > div.box__inner > h2 > a')
  ...
  this.Then(/^6 article descriptions visible$/, () => {
    const articleDescription = browser.elements('body > main > div > div.grid > article:nth-child(n) > div.box__inner > div.mb > p:first-of-type')
